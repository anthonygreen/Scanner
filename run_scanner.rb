...
s3.bucket('scanner.tools.bbc.co.uk').object('derp.html').upload_file('derp.html')
s3.bucket('scanner.tools.bbc.co.uk').object('derp.html').upload_file('derp.html')
s3.bucket('scanner.tools.bbc.co.uk').object('derp.html').acl.put({ acl: "public-read" })

# This works but doesn't load nav bar options
# cmd = `aws s3 cp derp.html s3://scanner.tools.bbc.co.uk/ --acl public-read`
